...
def heuristic_intent_router(query: str) -> IntentClassification:
    ...
    brujo_hits = sum(1 for term in brujo_terms if term in text)

    if brujo_hits > hr_hits and brujo_hits > tech_hits:
        return IntentClassification(
            intent=IntentLabel.BRUJO,
            confidence=0.85,
            rationale="Matched Magic/Astrology keywords",
        )

    if hr_hits > tech_hits and hr_hits > 0:
        return IntentClassification(
            intent=IntentLabel.HR, confidence=0.75, rationale="Matched HR keywords"
        )

    if tech_hits > hr_hits and tech_hits > 0:
        return IntentClassification(
            intent=IntentLabel.TECH, confidence=0.78, rationale="Matched TECH keywords"
        )

    return IntentClassification(
        intent=IntentLabel.UNKNOWN,
        confidence=0.45,
        rationale="Ambiguous or weak evidence",
    )
